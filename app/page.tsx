...
import { ArrowRight, ClipboardCheck, Landmark, ShieldCheck, Sparkles, Stars } from "lucide-react";
...
const trustHighlights = [
  ...
  {
    label: "Human + AI buying support",
    text: "Choose traditional agent guidance or an AI coordinator that accelerates your timeline.",
    icon: Stars
  },
  {
...
export default function LandingPage() {
  return (
    <div className="flex flex-col gap-16 pb-10">
      <section className="relative overflow-hidden rounded-[2.25rem] border border-border/70 bg-card px-8 py-12 shadow-soft md:px-14 md:py-16">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,_hsl(var(--primary)/0.22),_transparent_58%)]" aria-hidden />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-accent/40 via-accent/15 to-transparent" aria-hidden />
        <div className="relative z-10 flex max-w-4xl flex-col gap-8">
          <Badge className="w-fit gap-1 border border-primary/15 bg-primary/10 text-primary shadow-sm">
            ...
            Investor-ready global property
          </Badge>
          <div className="flex flex-col gap-5">
            <h1 className="text-5xl font-semibold tracking-tight text-foreground md:text-6xl">
              Buy property anywhere with a calm, legal-first edge.
            </h1>
            <p className="max-w-3xl text-lg leading-relaxed text-muted-foreground md:text-xl">
              Navigate cross-border purchases with guided legal steps, AI-assisted coordination, and currency clarity built into every search.
            </p>
          </div>
          ...
          <div className="hero-divider h-px w-40" aria-hidden />
          <div className="grid gap-4 pt-1 md:grid-cols-3">
            {trustHighlights.map((item) => {
              const Icon = item.icon;
              return (
                <Card key={item.label} className="h-full border-border/70 bg-card/95 shadow-sm">
      ...
      <section className="flex flex-col gap-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {featuredCities.map((city) => (
            <Card key={city.city} className="border-border/70 shadow-sm">
